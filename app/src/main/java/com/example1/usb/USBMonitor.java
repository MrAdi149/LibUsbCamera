...
import com.example.usbcamera.utils.FIleUtils;
import com.example1.utils.HandlerThreadHandler;
import com.example1.utils.BuildCheck;
...
//		if (DEBUG) Log.v(TAG, "getDeviceKeyName:" + sb.toString());
    ...
    public static UsbDeviceInfo updateDeviceInfo(final UsbManager manager, final UsbDevice device, final UsbDeviceInfo _info) {
        ...
        if (device != null) {
            ...
            if (TextUtils.isEmpty(info.manufacturer)) {
                info.manufacturer = USBVendroId.vendorName(device.getVendorId());
            }
            if (TextUtils.isEmpty(info.manufacturer)) {
